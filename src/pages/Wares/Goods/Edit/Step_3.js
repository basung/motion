...
export default class Index extends React.Component {
	...
	static getDerivedStateFromProps(nextProps, preState) {
		if (!isEqual(nextProps.currentItem, preState.newCurrentItem)) {
			...
			if (modalType === 'update' && !isEmpty(currentItem) && !isEmpty(currentItem.goodsAttributeList) && currentItem.goodsAttributeList.length > 0 ) {
				let goodsAttributeList = currentItem.goodsAttributeList;
				for(let index = 0; index < goodsAttributeList.length; index ++){
					goodsAttributeList[index].ids = goodsAttributeList[index].id
				}
				return {
					data : goodsAttributeList,
