	...
	generateSKU = (newData) => {
		...
		if (skuList != 0 && !isEmpty(skuList)) {
			for (let i = 0; i < skuList.length; i++) {
				...
				skuList[i].ids = 'SKU' + generateUUID();
				skuList[i].salePrice = '';
		...
		const columns = [
			{
				...
				width: '20%',
				render: (text, record) => {
					return (<Select style={{ width: '100%' }} defaultValue={text} onChange={e => this.handleChange(e, record.key, 'specId')} >{this.state.specChildren}</Select>);
				},
			}, {
				title: '规格值',
				...
				width: '70%',
				render: (text, record) => {
					return (<Select style={{ width: '100%' }} defaultValue={text} mode="tags" onChange={e => this.handleChange(e, record.key, 'specValueId')} >{this.state.specValue[record.key]}</Select>);
				},
			}, {
				title: '操作',
				dataIndex: 'operation',
