...
class modal extends React.Component {
	...
	state = {
		uploading: false,
		imageUrl: this.props.type === 'create' ? '' : this.props.item.logo,
	};
	...
	render() {
		...
		const modalOpts = {
			title: `${type === 'create' ? '新增咨询分类' : '编辑咨询分类'}`,
			visible,
			onOk: handleOk,
			onCancel,
				...
				<Form>
					...
					<FormItem {...formItemLayout} label="分类图片" >
						{getFieldDecorator('logo', {
							initialValue: (type === 'create') ? '' : item.logo,
							rules: [
								{
									required: false,
									message: '分类图片',
							...
							>
								{imageUrl ?
									...
									(type === 'create') ? uploadButton : <img src={IMGURL + item.logo} alt="avatar" width={'102px'} height={'102px'} />}
							</Upload>
						)}
					</FormItem>
